            ...
            <table class="table table-striped">
              ...
              <tr v-for="(v,i) in list" :key="i">
                ...
                <td>
                  ...
                  <a class="change-cursor" @click.prevent="deleteEquip(v.id,v.name)">删除</a>
                </td>
              </tr>
            </table>
        </div>
...
methods:{
  ...
  showAddVue(){
    ...
    this.$router.push({name:'addEquip'});// 找到路由配置中name为add的那项路由
  },
  showEditVue(currId){
    this.$router.push({name:'editEquip',params:{id:currId}});
  },
  deleteEquip(id,name){
      if(confirm('确定删除'+name+'吗？')){
        this.axios.delete('equip/'+id)
        .then((res) => {
          // 处理数据成功需要更新视图
          this.getData();
        });
